...
public partial class Application_Isys_ChannelMgmt_PopMemType : BaseClass
{
    #region DECLARATION
    ...
    protected void Binddatagrid()
    {
        try
        {
            ...
            //Added: Parag
            htParam.Add("@UnitCode",Convert.ToDecimal( Request.QueryString["untcd"].ToString().Trim()));
            htParam.Add("@RprtSetup", Request.QueryString["RptSetup"].ToString().Trim());
            ...
            if (Request.QueryString["untcd"].ToString() != "")
            {
                if (Request.QueryString["agttyp"] != "")
                {
                    ...
                    dsmgr = objDAL.GetDataSetForPrc("Prc_GetChnMapRptType", htParam);
