...
namespace INSCL
{
    public partial class AGTSearch : BaseClass
    {
        ...
        protected void dgDetails_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                if (dsMvmt.Tables.Count != 0)
                {
                    ...
                    {
                       // strMvmtLvl = dsMvmt.Tables[0].Rows[0]["MvmtLvl"].ToString().Trim(); commented by mrunal on 07.02.18 as maker and checker concept not required.
                    }
                }
                dsMvmt.Clear();
                        ...
                        if (lblAgentStatus.Text.Trim() != "IP")
                        {
                            ...
                            else
                            {
                                if (hdnAgentRole != null)
                                {
                                    ...
                                    {
                                        e.Row.Cells[1].Text = "<i class=\"fa fa-male\"></i>&nbsp;<a style=\"color:green;\"  href=\"AGTInfo.aspx?AgnCd=" + e.Row.Cells[1].Text + "&Type=" + ViewState["vwsType"].ToString() + "&ID=" + Request.QueryString["ID"].ToString() + "&UnitCode=" + e.Row.Cells[7].Text + "&ChnName=" + e.Row.Cells[7].Text+ "&MemTypeDesc=" + e.Row.Cells[8].Text+"\"><i class=\"fa fa-edit\" style=\"color:blue;\"></i>" + e.Row.Cells[1].Text + "</a>";
                                    }
                                }
                                else
                                {
        ...
        protected void rdbChnlTyp_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            ddlAgntType.Items.Insert(0, new ListItem("All", "All"));
           // rdbChnlTyp.Focus(); by meena 21/3/18
        } 
        #endregion
